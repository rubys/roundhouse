...
fn article_broadcasts_to_synthesizes_three_lifecycle_methods() {
    ...
    assert!(
        src.contains(
            "Broadcasts.remove(stream: \"articles\", target: \"#{self.dom_prefix}_#{self.dom_record_key}\")"
        ),
        "{src}",
    );
}
...
fn comment_broadcasts_rewrite_lambda_param_to_ivar() {
    ...
    assert!(
        src.contains("Broadcasts.replace(stream: \"article_#{@article_id}_comments\", target: \"#{self.dom_prefix}_#{self.dom_record_key}\""),
        "{src}",
    );
}
...
fn setter_send_renders_with_space_around_equals() {
    ...
    let files = lowered_real_blog();
    let src = find(&files, "article.rb");
    // `title` is a nullable string column, so the value carries Rails'
    // String cast (`lower::model_to_library::schema`) as the nil-safe
    // `attrs[:title]&.to_s`; the setter form is what this test pins.
    assert!(
