//! Go overlay regression tests.
//!
//! Locks in the contract for what's landed in `src/emit/go/`:
//!
...
//!
//!     cargo test --test go_smoke -- --ignored --nocapture
...
fn hand_written_runtime_present() {
    ...
    let files = go::emit_overlay_files(&app);
...
///    rust's `Self::X(args)` strategy. Subclass overrides don't
...
fn nil_check_to_comma_ok_rewrites_pair() {
    use roundhouse::emit::go::lower::{lower_for_go, nil_check_to_comma_ok};
...
fn nil_check_to_comma_ok_skips_non_hash_receiver() {
    use roundhouse::emit::go::lower::lower_for_go;
...
fn emit_to_scratch() -> PathBuf {
    let scratch = std::env::temp_dir().join("roundhouse-go-smoke");
    if scratch.exists() {
    ...
    files.extend(go::emit_overlay_files(&app));
