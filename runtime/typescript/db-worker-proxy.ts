...
// Nullable-column seam (see runtime/typescript/db.ts): NULL stays null
// rather than collapsing to ""/0, so a nullable UNIQUE column doesn't
// collide row-to-row and `where(fk: nil)` matches the rows that never
// set it.
