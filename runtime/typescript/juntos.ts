...
// Historical note: InMemoryActiveRecordAdapter + FrameworkTestAdapter
// previously lived here as the TS-side mirror of the Ruby
// `FrameworkTestAdapter` mock — exercised by base_test.rb under the
// framework_tests_typescript gate. Removed in tandem with the per-target
// adapter cleanup; a follow-on session will re-enable base_test wired
// against each target's real sqlite stack (TS: better-sqlite3 / libsql).
