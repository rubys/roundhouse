...
# server, built on Tep::Scheduler + sp_net non-blocking accept/recv.
#
# THE MEASUREMENT LANE. Tep::Server::Threaded (a green thread per
# connection) replaced this server on 2026-09-01; this file came back on
# 2026-09-03 so the two designs can be measured on ONE binary, differing
# only in the environment: `TEP_SERVER=fiber` selects it (scaffold/main.rb),
# `SPINEL_WORKERS` is irrelevant to it (every fiber runs on the main
# thread). It serves the HTTP keep-alive path only: a WebSocket upgrade
# is refused with a 501 (its recv loop belongs on a parked green thread,
# and here it would hold every other connection), and the per-request
# state the threaded server keeps in Thread.current is shared by all
# fibers on the main thread -- correct while a request runs from start to
# finish without yielding, which a GET does; so the bench pages run it and
# the campfire gates do not. matz/spinel#4306 is the question it answers.
#
# Why this exists
# ---------------
    ...
    class Scheduled
      ...
      def run(port, workers, quiet)
        ...
        if !quiet
          ...
          puts "  one fiber per connection (TEP_SERVER=fiber); OS workers: " +
               "1, the main thread; processes: " + Tep.processes_desc(workers)
          $stdout.flush
        end
        ...
        if workers > 1
          ...
          # Exit outright, as the threaded server does: the scaffold's
          # job-loop green thread is parked on its queue, and spinel runs
          # every remaining green thread to completion when main returns.
          Sock.sphttp_exit(0)
        end
        0
      end
      ...
      def self.write_response(client, req, res, keep_alive)
        ...
        if res.upgrading_ws
          # The fiber lane serves the HTTP path only: the driver's recv
          # loop wants a parked green thread (Tep::Server::Threaded), and
          # on this server it would run on the main thread and hold every
