...
//! `Expr` rendering.
//!
//! Forked 2026-05-21 from the original go expr emitter so go can
...
pub(super) fn emit_expr(ctx: &EmitCtx, e: &Expr) -> String {
    ...
    match &*e.node {
        ...
        // already performed by go's struct-embedding init
        ...
        // only the rhs, which loses the binding. go needs the real
...
pub(super) fn emit_send(
    ...
    // Mirrors rust's `try_view_helpers_dom_id` (src/emit/rust/
    ...
    // shape go emits as map/slice/interface{}), emit Go index-
    ...
    // `go_method_ident`-then-call dispatch (sees `!` as a method
    ...
    // the `interface{}` shape go emits for unknown types.
    ...
    // matching the rust `Self::X(args)` strategy. Subclass overrides
    ...
    let go_m = go_method_ident(method);
    match recv {
        None => {
...
/// this; method-CALL positions use `go_method_ident`, which affixes
...
fn go_method_ident(ruby_name: &str) -> String {
    // Mirror rust's `sanitize_ident` (src/emit/rust/expr/util.rs):
...
/// status: :ok, content_type: nil, location: nil)`. The rust target
/// has a generalized `GLOBAL_CLASS_METHOD_DEFAULTS` registry; go uses
...
/// registry once go grows one; this list is the minimum needed
...
mod predicate_naming_tests {
    use super::{go_method_ident, go_field_ident};
    ...
    fn call_path_affixes_field_path_strips() {
        ...
        assert_ne!(go_method_ident("exists"), go_method_ident("exists?"));
        assert_eq!(go_method_ident("exists?"), "ExistsPred");
        assert_eq!(go_method_ident("save!"), "SaveBang");
