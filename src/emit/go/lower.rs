...
//! and BEFORE go's emit walk. They rewrite IR shapes that translate
//! poorly to Go into shapes that go's emit can produce idiomatic Go
//! from.
//!
//! ## Why Go specifically
//!
...
//!
//! ## Relationship to rust's approach
//!
//! rust didn't need this layer — Rust's `Option<T>` and `Result<T, E>`
...
/// `emit::go::emit_overlay_files` via the `go_units` transform
/// hook — every transpiled framework class flows through this
/// pipeline before go/emit sees it.
...
pub mod nil_check_to_comma_ok {
    ...
    /// through go/emit's existing OpGet dispatch instead.
