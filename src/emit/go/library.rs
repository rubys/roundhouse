...
//!
//! Mirrors `src/emit/rust/library.rs` but emits Go. Couples the
...
/// ctor). Build the set in `go.rs::emit_overlay_files` by walking
...
pub fn emit_library_class_with_registry(
    ...
    // rust's `is_module_singleton` predicate.
    ...
    // Mirrors rust's per-model `pub fn exists/find/count/all/...`
        ...
        let go_ty = match m.kind {
            AccessorKind::AttributeReader => {
                if let Some(Ty::Fn { ret, .. }) = m.signature.as_ref() {
                    // Temporal reader (`Ty::Time` return): NO struct
                    // field. Storage is the `<col>_raw` String accessor
                    // pair from the shared lowering, which collapses to
                    // an ordinary `CreatedAtRaw string` field here;
                    // call-site reads of the Time-typed reader route to
                    // that field (the temporal fall-through in
                    // `expr::emit_send`) until Go's native `time.Time`
                    // seam is wired.
...
/// `go_method_ident` in expr.rs instead.
...
/// `go_method_ident` in expr.rs so call sites and method defs line
...
fn pascalize_instance_method_name(name: &str) -> String {
    ...
    // Mirror go_method_ident: `?` → `_pred`, `!` → `_bang`, `=` →
...
pub fn format_module_ivar(owner: &str, name: &str, value: &Expr) -> String {
    // ActionView::ViewHelpers's `@slots = {}` is owned by the hand-
    // written runtime/go/v2/slots.go (goroutine-keyed per-request
    // store). Suppress the auto-emit so there's no collision with the
    // hand-written `slotsByGoroutine` sync.Map. The six slot METHODS
    // are also dropped (see go_units transform callback in
    // src/emit/go.rs); together that retires the racy package-level
    // map + RWMutex pair from commit 1f2a984.
    if owner == "ActionView::ViewHelpers" && name == "slots" {
        return String::new();
    }
    let ctx = super::expr::EmitCtx::none();
    let rendered = super::expr::emit_expr(&ctx, value);
...
fn emit_method(
    ...
    let class_method_name = match m.receiver {
        ...
        // `go_method_ident` call-site shape (expr.rs). `destroyed?`
...
fn emit_module_singleton(class: &LibraryClass) -> Result<String, String> {
    ...
    //
    // The prior RWMutex emit + body-touches-slot wrap (commit
    // 1f2a984) targeted ActionView::ViewHelpers's slot store — the
    // only module-singleton whose @slots map is read+written from
    // parallel goroutines during request handling. That apparatus is
    // retired: the six slot methods are owned by hand-written
    // runtime/go/v2/slots.go (per-goroutine map keyed by goroutine ID
    // parsed from runtime.Stack), and the LibraryClass-level filter
    // in src/emit/go.rs::go_units drops the methods from the
    // ActionView::ViewHelpers class before this loop walks it.
    for m in &class.methods {
