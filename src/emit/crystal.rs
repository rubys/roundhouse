...
/// emitted Crystal file so Crystal's whole-program analysis sees the
/// full project. Sorted for determinism.
fn emit_app_cr(emitted: &[EmittedFile]) -> EmittedFile {
    let mut paths: Vec<String> = emitted
        .iter()
        .filter_map(|f| {
            let p = f.path.to_string_lossy();
            // Take only Crystal sources under `src/`. Strip `.cr`
            // extension (Crystal `require` doesn't want it) and the
            // `src/` prefix (require relative to app.cr's own dir).
    ...
    s.push_str("# Requires every emitted source so Crystal's whole-program\n");
    s.push_str("# analysis sees the full project. Order is alphabetical;\n");
    s.push_str("# Crystal handles forward references at type-inference time.\n\n");
    for p in &paths {
