...
fn emit_views_cr(app: &App) -> Vec<EmittedFile> {
    ...
    for view in &app.views {
        emit_cr_view_fn(&mut s, view, &known_models, &attrs_by_class);
...
fn emit_cr_view_fn(
    ...
    for line in emit_cr_view_body_walker(&rewritten, &ctx) {
        writeln!(out, "      {line}").unwrap();
    }
    writeln!(out, "    end").unwrap();
    writeln!(out, "  end").unwrap();
...
fn emit_cr_view_append(arg: &Expr, ctx: &CrViewCtx) -> String {
    ...
    if is_cr_simple_expr(inner, ctx) {
        return format!("io << ({}).to_s", emit_cr_view_expr_raw(inner, ctx));
    }
...
fn emit_cr_captured_helper(
    ...
    match method {
        "form_with" => {
            ...
            let mut lines = vec![
                "io << (begin".to_string(),
                "  __inner = String.build do |io|".to_string(),
                format!("    {pname} = Roundhouse::ViewHelpers::FormBuilder.new"),
            ];
            for line in emit_cr_view_body_walker(body, &inner_ctx) {
            ...
            lines.push(format!(
                "  Roundhouse::ViewHelpers.form_wrap(nil, {cls_expr}, __inner)"
            ));
            lines.push("end)".to_string());
            lines.join("\n")
        }
        "content_for" => "# content_for stashed".to_string(),
        _ => "io << \"\"".to_string(),
    }
}
...
fn emit_cr_render_call(arg: &Expr, ctx: &CrViewCtx) -> String {
    match &*arg.node {
        ...
        {
            // Association access on a record — Crystal models don't
            // emit has_many accessors yet, so the call would fail to
            // type-check. Placeholder until that lands.
            let _ = method;
            "io << \"\" # TODO ERB: render assoc (has_many emit)".to_string()
        }
        ...
        ExprNode::Send { recv: Some(r), method, args, .. } => {
            ...
            if args.is_empty() {
                if let ExprNode::Var { name, .. } | ExprNode::Ivar { name } = &*r.node {
                    if ctx.is_local(name.as_str()) {
                        match method_s {
                            "any?" | "present?" => return format!("!{name}.empty?"),
                            "none?" | "empty?" => return format!("{name}.empty?"),
                            _ => {}
                        }
                    }
                }
...
fn emit_controller(out: &mut String, c: &Controller, known_models: &[Symbol]) {
    ...
    // Declare every ivar referenced across actions/helpers at class
    // scope with a default. Crystal otherwise can't infer the type of
    // `@article` in actions that only read it (before_action filters
    // set it in the real Rails runtime; Phase 4c doesn't dispatch
    // filters). Default = fresh model or empty collection.
    let ivars = collect_controller_ivars(c, known_models);
    ...
    for action in &public_actions {
        ...
        first = false;
        emit_action(out, action, ctx, &ivars);
    }
    for helper in &private_actions {
        if !first {
            writeln!(out).unwrap();
        }
        first = false;
        emit_private_helper(out, helper);
    }
    writeln!(out, "end").unwrap();
}
...
fn emit_action(
    ...
    writeln!(out, "  def {name} : Roundhouse::Http::Response").unwrap();
    // Prime every referenced-but-never-assigned ivar with a default
    // at the start of the body. Class-level defaults handle this
    // statically, but repeating it here makes the intent explicit and
    // keeps each action hermetic — a future stage that wires real
    // filter dispatch can replace the priming with filter calls.
    let _ = ivars;
    let stmts = emit_action_stmts(&a.body, ctx);
    for line in stmts.lines() {
        writeln!(out, "    {line}").unwrap();
    }
    writeln!(out, "    Roundhouse::Http::Response.new").unwrap();
    writeln!(out, "  end").unwrap();
}

fn emit_private_helper(out: &mut String, a: &Action) {
    // Private helpers collapse to `<RetType>-default` bodies, same as
    // Rust: their Ruby semantics depend on the real params filter
    // which Phase 4c doesn't wire, and the `params.expect(...)` stub
    // can't honor every declared return type.
    let name = a.name.as_str();
    let ret = a.body.ty.clone().unwrap_or(Ty::Nil);
    let ret_annot = format!(" : {}", crystal_ty(&ret));
    writeln!(out, "  def {name}{ret_annot}").unwrap();
    writeln!(out, "    {}", crystal_default(&ret)).unwrap();
    writeln!(out, "  end").unwrap();
}
...
fn guess_ivar_type_cr(name: &str, known_models: &[Symbol]) -> String {
    ...
    if known_models.iter().any(|m| m.as_str() == singular_class) {
        if is_plural {
            return format!("Array({singular_class}) = [] of {singular_class}");
        }
...
// `crystal_default` already exists above — re-use it for Phase 4c's
// private helper bodies so the zero-value rendering stays consistent
// with model property defaults.

/// Controller-scope emit context. Threaded through `emit_expr_cr` via
/// the existing generic emit layer when `in_controller` is set.
...
/// Emit a controller action body as a sequence of Crystal statements.
/// Last expression's value is discarded; the action's `Response.new`
/// return is appended by the caller.
fn emit_action_stmts(body: &Expr, ctx: CrCtx) -> String {
    match &*body.node {
        ExprNode::Seq { exprs } if exprs.is_empty() => String::new(),
        ExprNode::Seq { exprs } => exprs
            .iter()
            .map(|e| emit_stmt_cr(e, ctx))
            .collect::<Vec<_>>()
            .join("\n"),
        _ => emit_stmt_cr(body, ctx),
    }
}

fn emit_stmt_cr(e: &Expr, ctx: CrCtx) -> String {
    match &*e.node {
        ExprNode::Assign { target: LValue::Var { name, .. }, value } => {
            format!("{} = {}", name, emit_expr_cr(value, ctx))
        }
        ExprNode::Assign { target: LValue::Ivar { name }, value } => {
            format!("@{} = {}", name, emit_expr_cr(value, ctx))
        }
        _ => emit_expr_cr(e, ctx),
    }
}

fn emit_expr_cr(e: &Expr, ctx: CrCtx) -> String {
    match &*e.node {
        ExprNode::Send { recv, method, args, block, .. } => {
            emit_send_cr(recv.as_ref(), method.as_str(), args, block.as_ref(), ctx)
        }
        ExprNode::If { cond, then_branch, else_branch } => {
            let cond_s = emit_expr_cr(cond, ctx);
            let then_s = emit_block_body_ctx_cr(then_branch, ctx);
            let else_s = emit_block_body_ctx_cr(else_branch, ctx);
            format!(
                "if {cond_s}\n{}\nelse\n{}\nend",
                indent(&then_s, 1),
                indent(&else_s, 1),
        ...
        // All other shapes fall back to the controller-unaware emitter;
        // they don't carry controller-specific surface (literals, hashes,
        // interpolated strings, etc.).
        _ => emit_expr(e),
    }
}

/// Render an expression as an indented block body — either its Seq of
/// statements or a single expression.
fn emit_block_body_ctx_cr(e: &Expr, ctx: CrCtx) -> String {
    match &*e.node {
        ExprNode::Seq { exprs } if !exprs.is_empty() => exprs
            .iter()
            .map(|stmt| emit_stmt_cr(stmt, ctx))
            .collect::<Vec<_>>()
            .join("\n"),
        _ => emit_stmt_cr(e, ctx),
    }
}
...
fn emit_send_cr(
    ...
    if ctx.in_controller {
        if let Some(s) = emit_controller_send_cr(recv, method, args, block, ctx) {
            return s;
        }
    }
    // Fall back to the controller-unaware path. Blocks and ctx don't
    // thread through the existing `emit_send`, which is fine for
    // remaining leaf cases (`x.destroy!`, chained property reads).
    let _ = block;
...
/// against `__fr`. Each statement is emitted via the controller-ctx
/// path so nested Sends pick up the format binding rewrite.
fn emit_respond_to_body_cr(body: &Expr, ctx: CrCtx) -> String {
    match &*body.node {
        ExprNode::Seq { exprs } => exprs
            .iter()
            .map(|e| emit_stmt_cr(e, ctx))
        ...
        ExprNode::If { cond, then_branch, else_branch } => {
            let cond_s = emit_expr_cr(cond, ctx);
            let then_s = emit_respond_to_body_cr(then_branch, ctx);
            let else_s = emit_respond_to_body_cr(else_branch, ctx);
            format!(
                "if {cond_s}\n{}\nelse\n{}\nend",
                indent(&then_s, 1),
                indent(&else_s, 1),
...
/// Render a block body (with or without a Lambda wrapper) as Crystal
/// source. Used for `format.html { ... }` inner blocks.
fn emit_block_body_ctrl_cr(e: &Expr, ctx: CrCtx) -> String {
