...
pub(crate) fn apply_create_block_inline(lcs: &mut [LibraryClass]) {
    for lc in lcs.iter_mut() {
        for m in &mut lc.methods {
            rewrite_create_block(&mut m.body);
        }
    }
}

fn rewrite_create_block(expr: &mut Expr) {
    expr.node.for_each_child_mut(&mut rewrite_create_block);
    let matches = matches!(
        &*expr.node,
        ExprNode::Send { method, args: _, block: Some(b), .. }
            if matches!(method.as_str(), "create" | "create!")
                && matches!(&*b.node, ExprNode::Lambda { params, .. } if params.len() == 1)
    );
    if !matches {
        return;
    }
