...
use super::expr::{emit_body, emit_literal};
...
pub(super) fn emit_py_views(app: &App) -> Vec<EmittedFile> {
    ...
    // Jbuilder (json-format) views → Python module functions + facade
    // entries. Computed up front so the header can pull in the aliased
    // `JsonBuilder` / `RouteHelpers` module imports their bodies reference.
    let (json_defs, json_facade) = emit_jbuilder_view_functions(app);
    let json_blob = json_defs.join("\n");
    ...
    if json_blob.contains("RouteHelpers") {
        writeln!(s, "from app import route_helpers as RouteHelpers").unwrap();
    }
    writeln!(s).unwrap();
    ...
    // Jbuilder view functions (`article_json`, `index_json`, `show_json`)
    // + their facade entries (`Views.Articles.index_json`). Emitted after
    // the html render functions so the facade's `staticmethod(...)` refs
    // resolve.
    for def in &json_defs {
        s.push_str(def);
        writeln!(s).unwrap();
    }
    for (resource, method, fn_name) in &json_facade {
        facade
            .entry(resource.clone())
            .or_default()
            .insert(method.clone(), fn_name.clone());
    }
...
/// Lower the json-format (jbuilder) views and render them as Python
/// module functions. Returns `(rendered_defs, facade_entries)` where
/// each facade entry is `(resource, method, fn_name)`. Mirrors
/// `emit::typescript`'s jbuilder path: the SAME shared lowering
/// (`lower_jbuilder_to_library_classes`), Python-rendered bodies. The
/// `view_lower_extras` are recomputed exactly as the model/view lowering
/// does (model registry + route-helper class infos) so the body-typer
/// resolves `article: Article`, `Views::Articles.article_json`, etc.
fn emit_jbuilder_view_functions(app: &App) -> (Vec<String>, Vec<(String, String, String)>) {
    let has_json = app.views.iter().any(|v| v.format.as_str() == "json");
    if !has_json {
    ...
    let mut extras: Vec<(crate::ident::ClassId, crate::analyze::ClassInfo)> =
        model_registry.into_iter().collect();
    extras.extend(route_helper_extras);

    let jbuilder_lcs = crate::lower::lower_jbuilder_to_library_classes(&app.views, app, extras);
    let funcs = crate::lower::flatten_lcs_to_functions(&jbuilder_lcs);

    let mut defs = Vec::new();
    let mut facade = Vec::new();
    for f in &funcs {
        defs.push(render_jbuilder_function(f));
        let resource = f
            .module_path
            .last()
        ...
        facade.push((resource, name.clone(), name));
    }
    (defs, facade)
}

/// Render one flattened jbuilder `LibraryFunction` as a Python `def`.
/// Params + return type come from the function's `Fn` signature (the
/// jbuilder lowering stamps `article: Article` etc.); the body — a
/// `String.new`/`<<`/terminal accumulator — renders through the shared
/// `emit_body`, whose `IrHint::StringBuilder*` handling turns it into a
/// `io = []` / `io.append(...)` / `"".join(io)` list-join.
fn render_jbuilder_function(f: &LibraryFunction) -> String {
    let (params, ret): (Vec<String>, Ty) = match &f.signature {
        Some(Ty::Fn { params, ret, .. }) if params.len() == f.params.len() => {
            let ps = f
                .params
                .iter()
                .zip(params.iter())
                .map(|(name, p)| {
                    format!("{}: {}", super::shared::py_ident(name.as_str()), python_ty(&p.ty))
                })
                .collect();
            (ps, (**ret).clone())
        }
        _ => (
            f.params.iter().map(|p| super::shared::py_ident(p.as_str())).collect(),
            Ty::Str,
        ),
    };
    let mut out = format!(
        "def {}({}) -> {}:\n",
        super::shared::py_ident(f.name.as_str()),
        params.join(", "),
        python_ty(&ret),
    );
    let body = emit_body(&f.body, &ret);
