//! Output-path routing for the elixir overlay.
//!
//! Mirrors `src/emit/go/paths.rs`: every emitted file flows through
...
/// Logical kind of an emitted elixir file.
pub(crate) enum OutputKind<'a> {
    ...
    /// with no useful Ruby source to transpile). Mirrors go's
    /// `HandWrittenRuntime`.
    ...
    // Constructed by the gated Phase C controller-emit path (elixir
...
/// Output destination for a file emitted by the elixir overlay.
