...
pub(super) fn emit_expr(ctx: &EmitCtx, e: &Expr) -> String {
    match &*e.node {
        ...
        // generated by `format_module_ivar`.
        ExprNode::Ivar { name } => {
...
fn emit_assign(ctx: &EmitCtx, target: &crate::expr::LValue, value: &Expr) -> String {
    ...
    match target {
        ...
        LValue::Ivar { name } => {
            // Same scoping rule as Ivar reads: class methods write
            // through to the bare lowercase package var; instance
            // methods write through `self.Field` (PascalCased).
            if ctx.in_class_method {
