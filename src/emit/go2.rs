...
pub fn emit_overlay_files(app: &App) -> Vec<EmittedFile> {
    ...
    for unit in &units {
        ...
        let out_path = output_path(OutputKind::TranspiledRuntime {
            file_name: &file_name,
        })
        .path;
        let content = rewrite_package_to_v2(&unit.content, &FileImports::new());
        out.push(EmittedFile {
            path: out_path,
            content,
        });
...
\t})\n\
        ...
        for lc in &lowered_models {
            ...
            let content = rewrite_package_to_v2(&raw, &FileImports::new());
            out.push(EmittedFile {
                path: output_path(OutputKind::Model {
        ...
        for lc in &lowered_controllers {
            ...
            let content = rewrite_package_to_v2(&raw, &FileImports::new());
            out.push(EmittedFile {
                path: output_path(OutputKind::Controller {
                    lc_name: lc.name.0.as_str(),
                })
                .path,
                content,
        ...
        for lc in &lowered_route_helpers {
            ...
            let content = rewrite_package_to_v2(&raw, &FileImports::new());
            out.push(EmittedFile {
                path: output_path(OutputKind::RouteHelpers).path,
                content,
            });
        }
        ...
        for lc in &lowered_importmap {
            ...
            let content = rewrite_package_to_v2(&raw, &FileImports::new());
            out.push(EmittedFile {
                path: output_path(OutputKind::Importmap).path,
                content,
            });
        }
        ...
        for lc in &lowered_views {
            ...
            let content = rewrite_package_to_v2(&raw, &FileImports::new());
            out.push(EmittedFile {
                path: output_path(OutputKind::ViewBundle {
                    resource_snake: &stem,
                })
                .path,
                content,
...
/// than the full `rewrite_package_to_v2` plumbing.
///
...
fn emit_v2_test_support(_app: &App) -> EmittedFile {
    let content = r##"// Generated by Roundhouse (go2 Phase 6 step 1).
// Test-support runtime for `go test ./app/v2/` — Dispatches through
// the same v2 router/controller path production traffic uses. Mirrors
// the legacy `app/test_support.go` API surface (TestClient.Get/Post/
// Patch/Put/Delete, TestResponse.AssertOk/AssertSelect/...) so the
// legacy spec.rs emit doesn't have to know it's targeting v2.
...
/// and per-LC emit functions) to `package v2`, and inject a per-file
/// `import (…)` block.
///
...
/// `gofmt`-compatible).
fn rewrite_package_to_v2(content: &str, extra: &FileImports) -> String {
