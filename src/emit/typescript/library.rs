...
pub(super) fn emit_class_file_full(
    ...
    let imports = render_imports_with_companions(
        lc, app, &out_path, synthesized, companions,
    );
    if !imports.is_empty() {
        s.push_str(&imports);
        writeln!(s).unwrap();
    ...
    if !constants.is_empty() {
        for (name, value) in constants {
            ...
            // minimal `[]`-style accessor and a `.name` getter so
            // bare-name refs in test methods resolve. The block body's
            // own statements are dropped (they expand to instance
            // methods on the struct, not handled yet).
            if let crate::expr::ExprNode::Send { recv, method, args, .. } = &*value_expr.node {
                ...
                if recv_is_struct && method.as_str() == "new" {
                    let fields: Vec<&str> = args
                        ...
                        .collect();
                    if !fields.is_empty() {
                        writeln!(s, "class {} {{", name.as_str()).unwrap();
                        write!(s, "  constructor(").unwrap();
                        let params = fields
                            .iter()
                            .map(|f| format!("public {f}: any"))
                            .collect::<Vec<_>>()
                            .join(", ");
                        s.push_str(&params);
                        writeln!(s, ") {{}}").unwrap();
                        // Ruby `record[:field]` translates to `record.get(:field)`
                        // in the TS emit (HWIA-like dispatch); provide a `get`
                        // method that maps to the named field.
                        writeln!(s, "  get(field: string): any {{").unwrap();
                        writeln!(s, "    return (this as any)[field];").unwrap();
                        writeln!(s, "  }}").unwrap();
                        // `self.name` getter — `Article.name` returns the
                        // class's bare name. JS classes already expose
                        // `Article.name` natively, but a Ruby override
                        // (`def self.name = "Article"`) is the common idiom;
                        // the native value matches what the override sets.
                        writeln!(s, "}}").unwrap();
                        continue;
                    }
                }
            }
            let value_s = super::expr::emit_expr(value_expr);
            writeln!(s, "const {} = {};", name.as_str(), value_s).unwrap();
        }
        writeln!(s).unwrap();
    }

    for companion in companions {
        match super::emit_library_class(companion) {
            Ok(body) => {
                s.push_str(&body);
                s.push('\n');
            }
...
const JUNTOS_EXPORTS: &[&str] = &[
    ...
    // Test-only singleton mirroring `runtime/ruby/test/test_helper.rb`
    // `FrameworkTestAdapter`. Framework tests assign it to
    // `ActiveRecord.adapter` and call `reset_all_bang` /
    // `create_table` / `insert` directly.
...
fn render_imports_with_synthesized(
    ...
    render_imports_with_companions(lc, app, out_path, synthesized, &[])
}

fn render_imports_with_companions(
    lc: &LibraryClass,
    app: &App,
    out_path: &std::path::Path,
    synthesized: &[String],
    ...
    let mut validations_import: bool = false;
    ...
    for r in refs {
        ...
        if JUNTOS_EXPORTS.contains(&r_str) {
            ...
            // `ActiveRecord.adapter = FrameworkTestAdapter` and
            // dispatches through `ActiveRecord.adapter.<method>`
            // call sites the framework Ruby produces. Ride along
            // on the active_record_base import below.
            active_record_namespace_import = true;
        } else if r == "Validations" {
            // Transpiled `ActiveRecord::Validations` mixin — emitted
            // to `src/validations.ts`. Inline framework-test classes
            // can `include ActiveRecord::Validations`, which lowers
            // to `extends Validations`. Without this import the
            // hoisted class trips ReferenceError at module load.
            validations_import = true;
    ...
    if active_record_base_import || active_record_namespace_import {
        ...
        writeln!(s, "import {{ {names} }} from \"{runtime_path}\";").unwrap();
    }
    if validations_import {
        let runtime_path = relative_to_root(out_path, "src/validations.js");
        writeln!(s, "import {{ Validations }} from \"{runtime_path}\";").unwrap();
    }
