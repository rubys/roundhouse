...
use super::js_ast::{
    Js, JsClassMember, JsDecl, JsExpr, JsImport, JsModule, JsParam, JsStmt, JsStmtNode,
    MethodKind, TsType,
};
...
fn module_header() -> Vec<String> {
    vec!["Generated by Roundhouse.".to_string()]
}
...
pub(super) fn emit_module_file(
    ...
    let imports = collect_imports(&synthetic_lc, app, &out_path);

    let mut decls: Vec<JsDecl> = Vec::new();
    let mut exported_names: Vec<(String, String)> = Vec::new();
    // (key_in_namespace, exported_identifier)
    for func in funcs {
        match super::js_library_function(func) {
            Ok(d) => decls.push(d),
            Err(e) => decls.push(JsDecl::Raw(format!("// EMIT ERROR: {e}"))),
        }
    ...
    // Rendered as pre-formed text: the multi-line shorthand object is
    ...
    {
        let mut text = format!("export const {module_name} = {{\n");
        for (i, (key, exported)) in exported_names.iter().enumerate() {
            let comma = if i + 1 < exported_names.len() { "," } else { "" };
            if key == exported {
                text.push_str(&format!("  {key}{comma}\n"));
            } else {
                text.push_str(&format!("  {key}: {exported}{comma}\n"));
            }
        }
        text.push_str("};");
        decls.push(JsDecl::Raw(text));
    }
...
pub(super) fn emit_function_file(
    ...
    let decls = vec![match super::js_library_function(func) {
        Ok(d) => d,
        Err(e) => JsDecl::Raw(format!("// EMIT ERROR: {e}")),
    }];
    render_module(
        &JsModule { header: module_header(), imports, decls },
        out_path,
...
pub(super) fn emit_views_aggregator(
    ...
    let mut imports: Vec<JsImport> = Vec::new();
    for (alias, import_path, fn_name) in &alias_map {
        ...
        let alias_opt = if &escaped_fn == alias { None } else { Some(alias.clone()) };
        imports.push(JsImport::Named {
            names: vec![(escaped_fn, alias_opt)],
            from: import_path.clone(),
        });
    }
    // The nested namespace object is synthesized glue (no source
    // spans); rendered as pre-formed text.
    let mut text = String::from("export const Views = ");
    write_namespace_object(&mut text, &grouped, &[], 0);
    text.push(';');
...
fn collect_imports_for_function(
    func: &crate::dialect::LibraryFunction,
    app: &App,
    out_path: &std::path::Path,
) -> Vec<JsImport> {
    // Synthesize a one-method LibraryClass so we can reuse the class
    // import collector verbatim. The synthesized class never reaches
    // the file output — only its method body is walked for refs.
...
/// Variant that emits one or more companion classes inline above the
/// primary class in the same file. Used by the test-file emit path
/// to hoist inline classes (e.g. `Validatable` declared inside
/// `class ValidationsTest`) to file scope while sharing the import
/// header — the union of class references from primary + companions
/// drives the import set so all referenced names resolve.
pub(super) fn emit_class_file_with_companions(
    lc: &LibraryClass,
    app: &App,
    out_path: PathBuf,
    synthesized: &[String],
...
/// Same as `emit_class_file_with_companions` plus a list of file-scope
/// `const NAME = <expr>` declarations to emit after the imports and
/// before any class bodies. Used by the test emit to lift class-body
/// constants (`TABLE = [...]`, `Article = Struct.new(...)`) declared
/// inside the test class to file scope, matching Ruby's lexical
/// constant resolution for bare references inside test methods.
pub(super) fn emit_class_file_full(
    lc: &LibraryClass,
    app: &App,
    out_path: PathBuf,
    synthesized: &[String],
    ...
    let imports = collect_imports_with_companions(lc, app, &out_path, synthesized, companions);

    let mut decls: Vec<JsDecl> = Vec::new();
    for (name, value) in constants {
        // Strip a trailing `.freeze` send — Ruby's `.freeze` has
        // no TS analog and the immutable literal it wraps is the
        // intended TS value.
        ...
        // `NAME = Struct.new(:a, :b, :c) do ... end` — synthesize a
        // TS class with positional constructor params bound to
        // public fields. The block body's `def [](field)` /
        // `def self.name` are common stand-in idioms; emit a
        // minimal `[]`-style accessor so bare-name refs in test
        // methods resolve. The block body's own statements are
        // dropped (they expand to instance methods on the struct,
        // not handled yet).
        if let crate::expr::ExprNode::Send { recv, method, args, .. } = &*value_expr.node {
            ...
            if recv_is_struct && method.as_str() == "new" {
                let fields: Vec<&str> = args
                    ...
                    .collect();
                if !fields.is_empty() {
                    decls.push(struct_class_decl(name.as_str(), &fields));
                    continue;
                }
            }
        }
        decls.push(JsDecl::Const {
            export: false,
            name: name.as_str().to_string(),
            ty: None,
            value: super::expr::js_expr(value_expr),
            span: value_expr.span,
        });
    }

    for companion in companions {
        match super::js_library_class(companion) {
            Ok(d) => decls.push(d),
            Err(e) => decls.push(JsDecl::Raw(format!(
                "// EMIT ERROR for {}: {e}",
                companion.name.0.as_str(),
            ))),
        }
    }

    match super::js_library_class(lc) {
        Ok(d) => decls.push(d),
        Err(e) => decls.push(JsDecl::Raw(format!("// EMIT ERROR: {e}"))),
    }
...
/// `NAME = Struct.new(:a, :b)` shim — a class whose constructor binds
/// positional args to public fields, plus the `get(field)` accessor
/// the `record[:field]` emit dispatches to. `Article.name` is the JS
/// class's native `.name`, matching the common Ruby override.
fn struct_class_decl(name: &str, fields: &[&str]) -> JsDecl {
    let ctor_params: Vec<JsParam> = fields
        .iter()
        .map(|f| JsParam {
            // `public <f>` is TS parameter-property syntax — binding
            // text, carried opaquely in the name slot.
            name: format!("public {f}"),
            optional: false,
            ty: Some(TsType("any".into())),
            default: None,
        })
        .collect();
    let get_body = vec![JsStmt::synth(JsStmtNode::Return(Some(Js::synth(JsExpr::Index {
        obj: Js::synth(JsExpr::Cast {
            expr: Js::synth(JsExpr::Ident("this".into())),
            ty: TsType("any".into()),
        }),
        index: Js::synth(JsExpr::Ident("field".into())),
    }))))];
    JsDecl::Class {
        export: false,
        name: name.to_string(),
...
/// Names that resolve to single-file runtime-src modules under
/// `src/<stem>.ts`. Each entry pairs the imported name with the
/// runtime stem it's exported from. Tests + framework code that
/// reference these by their qualified Ruby path
/// (`ActiveRecord::RecordNotFound`, `ActionController::Parameters`,
/// `ActionDispatch::Router`) collapse to the bare name via the
/// framework-namespace stripper in `collect_class_refs`/`emit_const`,
...
fn collect_imports(lc: &LibraryClass, app: &App, out_path: &std::path::Path) -> Vec<JsImport> {
    collect_imports_with_companions(lc, app, out_path, &[], &[])
}

fn collect_imports_with_companions(
    lc: &LibraryClass,
    app: &App,
    out_path: &std::path::Path,
    synthesized: &[String],
    ...
    if let Some(p) = &lc.parent {
        let import_name = parent_to_ref(p.0.as_str());
        if !import_name.is_empty() {
            refs.insert(import_name);
        }
    }
    // Companions — inline classes hoisted into the same file. Their
    // method bodies and parent references add to the import set so
    // hoisted classes' dependencies (`Validations`, `Base`, etc.)
    // resolve. The companion class names themselves get stripped
    // below since they're file-local.
    for companion in companions {
        for m in &companion.methods {
            collect_class_refs(&m.body, &mut refs);
            if let Some(sig) = &m.signature {
                collect_ty_class_refs(sig, &mut refs);
            }
        ...
        for inc in &companion.includes {
            let import_name = parent_to_ref(inc.0.as_str());
            if !import_name.is_empty() {
                refs.insert(import_name);
            }
        }
    ...
    // (name, stem) for each src/<stem>.ts runtime-src module reference.
    let mut runtime_src_imports: Vec<(String, String)> = Vec::new();
    ...
    let mut active_record_namespace_import: bool = false;
    ...
    for r in refs {
        ...
        if JUNTOS_EXPORTS.contains(&r_str) {
        ...
        } else if let Some(&(_, stem)) = RUNTIME_SRC_IMPORTS.iter().find(|(n, _)| *n == r_str) {
            runtime_src_imports.push((r, stem.to_string()));
            ...
            // `adapter` slot. Framework-level test code assigns
            // `ActiveRecord.adapter = SomeAdapter` and dispatches
            // through `ActiveRecord.adapter.<method>` call sites the
            // framework Ruby produces. Ride along on the
            // active_record_base import below.
            active_record_namespace_import = true;
    ...
    let named = |names: Vec<String>, from: String| JsImport::Named {
        names: names.into_iter().map(|n| (n, None)).collect(),
        from,
    };
    let mut out: Vec<JsImport> = Vec::new();
    if !juntos_imports.is_empty() {
        out.push(named(juntos_imports, "juntos".to_string()));
    }
    if !runtime_imports.is_empty() {
        ...
        out.push(named(runtime_imports, runtime_path));
    }
    // Runtime-src single-file modules — group by stem so multiple
    // imports from `src/errors.ts` (`RecordNotFound`, `RecordInvalid`)
    // share one import statement.
    {
        use std::collections::BTreeMap;
        let mut grouped: BTreeMap<String, Vec<String>> = BTreeMap::new();
        for (n, stem) in &runtime_src_imports {
            grouped.entry(stem.clone()).or_default().push(n.clone());
        }
        for (stem, names) in grouped {
            let import_path = relative_to_root(out_path, &format!("src/{stem}.js"));
            out.push(named(names, import_path));
        }
    }
    for (n, stem) in &model_imports {
        ...
        out.push(named(vec![n.clone()], import_path));
    }
    ...
    if controller_base_import {
        ...
        out.push(named(vec!["Base".to_string()], runtime_path));
    }
    if active_record_base_import || active_record_namespace_import {
        ...
        if active_record_namespace_import {
            names.push("ActiveRecord".to_string());
        }
        out.push(named(names, runtime_path));
    }
    if route_helpers_import {
        ...
        out.push(named(vec!["RouteHelpers".to_string()], import_path));
    }
    if views_import {
        ...
        out.push(named(vec!["Views".to_string()], import_path));
    }
    if view_helpers_import {
        let import_path = relative_to_root(out_path, "src/view_helpers.js");
        out.push(named(vec!["ViewHelpers".to_string()], import_path));
    }
    if inflector_import {
        ...
        out.push(JsImport::Star { alias: "Inflector".to_string(), from: import_path });
    }
    ...
    if broadcasts_import {
        ...
        out.push(named(vec!["Broadcasts".to_string()], import_path));
    }
    if importmap_import {
