...
/// Wires the transpiled-and-stub axum `router::router()` into
/// `server::start` with the schema DDL + a `layout: None` (the
/// rust2 `Layouts::application(body, notice, alert)` signature
/// doesn't match the server's `fn() -> String` slot; the runtime
/// falls back to the minimal synthesized shell until the layout
/// bridge lands in a follow-on wedge).
///
...
fn emit_main_rs(_app: &App) -> String {
