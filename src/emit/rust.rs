...
fn emit_rust_fixture(fixture: &Fixture, app: &App) -> EmittedFile {
    ...
    // of every test. Inserts each YAML record via the model's `save`
    // (so validations apply), captures the AUTOINCREMENT id, and
    // records it in the shared FIXTURE_IDS map so associated fixtures
    // and test getters can look each record up by label.
    writeln!(s).unwrap();
    writeln!(s, "pub fn _load_all() {{").unwrap();
    for (label, fields) in fixture.records.iter() {
        ...
        writeln!(
            s,
            "    assert!(r.save(), \"fixture {}/{} failed to save\");",
            fixture.name.as_str(),
            label.as_str(),
        )
        .unwrap();
        writeln!(
            s,
            "    crate::fixtures::FIXTURE_IDS.with(|m| {{ m.borrow_mut().insert(({:?}, {:?}), r.id); }});",
            fixture.name.as_str(),
            label.as_str(),
        )
        .unwrap();
    ...
    for (label, _fields) in fixture.records.iter() {
        writeln!(s).unwrap();
        writeln!(s, "pub fn {}() -> {} {{", label.as_str(), class_name).unwrap();
        writeln!(
            s,
            "    let id = crate::fixtures::fixture_id({:?}, {:?});",
            fixture.name.as_str(),
            label.as_str(),
        )
        .unwrap();
        writeln!(
            s,
            "    {}::find(id).expect(\"fixture {}/{} not loaded — call crate::fixtures::setup() first\")",
            class_name,
            fixture.name.as_str(),
            label.as_str(),
        )
            ...
            if name == field {
                    ...
                    .find(|f| f.name.as_str() == target_table.as_str())
                {
                    if target_fixture
                        .records
                        .keys()
                        .any(|k| k.as_str() == value)
                    {
                        // Runtime lookup — the referenced fixture's
                        // actual DB id is only known after _load_all
                        // has inserted it. Emitting the lookup keeps
                        // cross-fixture refs stable regardless of
                        // AUTOINCREMENT ordering quirks.
                        let resolved = format!(
                            "crate::fixtures::fixture_id({:?}, {value:?})",
                            target_table.as_str(),
                        );
                        return Some((foreign_key.as_str().to_string(), resolved));
                    }
                }
            }
        }
