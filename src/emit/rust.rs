...
fn emit_view_append(arg: &Expr, ctx: &ViewEmitCtx, buf_name: &str) -> Vec<String> {
    ...
    // `turbo_stream_from "channel"` — bare helper call; runtime
    // produces the Turbo cable-subscription tag.
    if let ExprNode::Send { recv: None, method, args, block: None, .. } = &*inner.node {
        if method.as_str() == "turbo_stream_from" && args.len() == 1 {
            if let Some(chan) = emit_view_string_arg(&args[0], ctx) {
                return vec![format!(
    ...
    // `form.label("title", &opts)`. `form` must be a view-scope
    // local (yielded by the surrounding form_with's block param).
    if let ExprNode::Send { recv: Some(r), method, args, block: None, .. } = &*inner.node {
        if let ExprNode::Var { name: recv_name, .. } = &*r.node {
            if ctx.is_local(recv_name.as_str()) {
                let rs_method = match method.as_str() {
                    "label" => Some("label"),
                ...
                if let Some(rs_method) = rs_method {
...
fn emit_captured_block_helper(
    ...
    // Resolve the record reference the form is bound to. Prefer the
    // `model:` kwarg when it's a simple expression; otherwise fall
    // back to the view fn's primary arg (Rails partial convention —
    // `_form.html.erb(article)` implicitly forms on `article`).
    let model_expr = extract_kwarg(args, "model");
    let record_ref: Option<String> = model_expr
        .filter(|e| is_simple_view_expr(e, ctx))
        .map(|e| emit_view_expr_raw(e, ctx))
        .or_else(|| {
    ...
    match method {
        "form_with" => {
            ...
            let (resource_path_expr, is_persisted_expr) = match &record_ref {
                Some(record) => (
                    format!(
...
fn emit_rust_form_builder_call(
    ...
    match rs_method {
        ...
        "submit" => Some(format!("{recv}.submit(&{opts})")),
        _ => None,
    }
}
...
/// expression. Only handles entries with scalar string values —
/// array-shaped `class: [str, {cond: ...}]` is simplified to its
/// first string element (matches the TS emitter's lowering).
