//! Elixir target — the live Elixir emitter (lowered IR + transpiled
//! `runtime/ruby/`).
//!
//! Built as a strangler-fig (`elixir2`) via the go/rust migration
//! pattern; the legacy v1 app-shell emitters were retired in Phase D3
//! and the module assumed the `elixir` name once the shim retired.
//! `entry` holds the public `emit` (project shell + overlay) that
//! lived in the old shim.
//!
//! The overlay emits transpiled framework-runtime + app modules under
//! `lib/v2/` in a dedicated `V2.*` Elixir namespace, alongside the
//! hand-written `Roundhouse.*` primitives (`runtime/elixir/`). The
//! runtime table (`ELIXIR_RUNTIME` in `runtime_loader.rs`) names the
//! transpiled slice; the per-variant body walker lives in `expr.rs`.
//!
...
//! mutation is threaded through return values by the
//! `lower::functionalize` passes, which only this target invokes.
...
/// Append the elixir transpiled-runtime overlay to `files`.
pub fn overlay_v2(files: &mut Vec<EmittedFile>, app: &App) {
    files.extend(emit_overlay_files(app));
}

/// Produce the elixir overlay files. Phase 1: just the transpiled
...
pub fn emit_overlay_files(app: &App) -> Vec<EmittedFile> {
    ...
    let units = match crate::runtime_loader::elixir_units(|_ns, classes| {
        ...
        Err(e) => {
            ...
            // sees a non-empty (failing) result. Mirrors go.
            out.push(EmittedFile {
                path: output_path(OutputKind::TranspileError).path,
                content: format!("# elixir2 transpile failed: {e}\n"),
            });
    ...
    // go's `db.go` / rust's `db.rs` hand-written-runtime rationale.
    if !app.models.is_empty() {
        ...
        // Mirrors go's lower-all → register-all → emit-all order
        // (go.rs:313-400). Gated on views (their only consumer today;
        ...
        if views_enabled {
            ...
            // `Views::<Resource>` module per resource — go.rs:369).
        ...
        // from the same `controller_to_library` pipeline go/rust use.
...
/// concrete controller (mirrors go's `emit_dispatch_file`): construct the
...
/// functions. Mirrors `go.rs::module_funcs_to_library_class`.
