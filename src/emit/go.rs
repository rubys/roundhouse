...
/// Minimal `go.mod` — module name `app`, targeting Go 1.24. No
/// dependencies yet; when runtime lands (HTTP server, DB adapter),
/// require directives go here.
...
fn emit_models(app: &App) -> EmittedFile {
    ...
    for model in &app.models {
        ...
        if !lowered.is_empty() {
            ...
            writeln!(body).unwrap();
            emit_save_method_go(&mut body, model);
        }
    }
...
fn emit_save_method_go(out: &mut String, model: &Model) {
    let recv_var = receiver_letter(model);
    writeln!(
        out,
        "func ({recv_var} *{}) Save() bool {{",
        model.name.0
    )
    .unwrap();
    writeln!(out, "\treturn len({recv_var}.Validate()) == 0").unwrap();
    writeln!(out, "}}").unwrap();
}
