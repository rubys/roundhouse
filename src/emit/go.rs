//! Go target — the live Go emitter.
//!
//! Built as a strangler-fig (`go2`) via the rust migration pattern,
//! became the sole Go emit path at the Phase 6 switchover, and
//! assumed the `go` name once the legacy shim retired. `entry` holds
//! the public `emit` assembly and the standalone `emit_method`
//! extraction walker that lived in the old shim.
//!
...
pub use entry::{emit, emit_method};
...
/// Append go transpiled runtime files to `files`. Phase 6 step 2
...
pub fn emit_overlay_files(app: &App) -> Vec<EmittedFile> {
    ...
    if !app.models.is_empty() {
        ...
        // legacy main.go's package main. Mirrors rust's `src/main.rs`
...
\t})\n\
        ...
        // Mirrors rust.rs / typescript.rs / crystal.rs.
        let params_specs =
        ...
        // `interface{}` instead of `*Article`. Mirrors the rust wiring
        // (src/emit/rust.rs:402) — the previous `class_info_from_library_class`
        ...
        // Views — real emit (Phase 4 wedge 16). Mirrors rust pattern
        // (src/emit/rust.rs:435-487): lower HTML + JBuilder variants
        ...
        let lowered_views: Vec<crate::dialect::LibraryClass> = if !app.views.is_empty() {
            ...
            // Merge HTML + JSON variants by struct name (rust:462-479).
        ...
        // (lowered above). Mirrors rust's view-emit pattern
        // (src/emit/rust.rs:743-).
        ...
        // bodies resolve without per-test re-emit. Mirrors rust wedge
