...
use crate::App;
use crate::dialect::{Action, Controller, MethodDef, Model, RouteSpec, Test, TestModule};
...
pub fn emit(app: &App) -> Vec<EmittedFile> {
    ...
    if !app.controllers.is_empty() {
        ...
        for controller in &app.controllers {
            files.push(emit_controller_pass2(controller, &known_models));
        }
...
fn emit_controller_pass2(c: &Controller, known_models: &[Symbol]) -> EmittedFile {
    let name = c.name.0.as_str();
    let resource = resource_from_controller_name_go(name);
    let model_class = crate::naming::singularize_camelize(&resource);
    let has_model = known_models.iter().any(|m| m.as_str() == model_class);
    let parent = find_nested_parent_go(name);
    let permitted = permitted_fields_for_go(c, &resource)
        .unwrap_or_else(|| default_permitted_fields_go(&model_class));
...
fn resource_from_controller_name_go(name: &str) -> String {
    let trimmed = name.strip_suffix("Controller").unwrap_or(name);
    crate::naming::singularize(&crate::naming::snake_case(trimmed))
}

#[derive(Clone, Debug)]
struct GoNestedParent {
    singular: String,
    #[allow(dead_code)]
    plural: String,
}

fn find_nested_parent_go(controller_name: &str) -> Option<GoNestedParent> {
    let resource = resource_from_controller_name_go(controller_name);
    if resource == "comment" {
        Some(GoNestedParent {
...
fn permitted_fields_for_go(c: &Controller, resource: &str) -> Option<Vec<String>> {
    ...
    extract_permitted_from_expr_go(&action.body)
}

fn extract_permitted_from_expr_go(expr: &Expr) -> Option<Vec<String>> {
    ...
    if let ExprNode::Seq { exprs } = &*expr.node {
        for e in exprs {
            if let Some(v) = extract_permitted_from_expr_go(e) {
                return Some(v);
            }
        }
    }
...
fn default_permitted_fields_go(model_class: &str) -> Vec<String> {
    match model_class {
...
fn emit_go_route_helpers(app: &App) -> EmittedFile {
    let flat = flatten_go_routes(app);
    let mut s = String::new();
    writeln!(s, "// Generated by Roundhouse.").unwrap();
    writeln!(s, "package app").unwrap();
    writeln!(s).unwrap();
...
fn flatten_go_routes(app: &App) -> Vec<GoFlatRoute> {
    let mut out = Vec::new();
    for entry in &app.routes.entries {
        collect_flat_go_routes(entry, &mut out, None);
    }
    out
}

fn collect_flat_go_routes(
    spec: &RouteSpec,
    out: &mut Vec<GoFlatRoute>,
    scope_prefix: Option<(&str, &str)>,
) {
    match spec {
        RouteSpec::Explicit { method, path, controller, action, as_name, .. } => {
            let (full_path, mut params) = nest_go_path(path, scope_prefix);
            extract_go_path_params(&full_path, &mut params);
            let m = match method {
            ...
            let handler = go_action_handler_name(controller.0.as_str(), action.as_str());
            out.push(GoFlatRoute {
                method: m.to_string(),
                path: full_path,
                handler,
                as_name: as_name
        ...
        RouteSpec::Root { target } => {
            let (controller, action) = target
                .split_once('#')
                .map(|(c, a)| (controller_class_name_go(c), a.to_string()))
                .unwrap_or_else(|| (target.clone(), "index".to_string()));
            let handler = go_action_handler_name(&controller, &action);
            out.push(GoFlatRoute {
        ...
        RouteSpec::Resources { name, only, except, nested } => {
            ...
            let controller = controller_class_name_go(name.as_str());
            for (action, method, suffix) in standard_resource_actions() {
                ...
                let as_name = go_resource_as_name(
                    action,
                    &singular_low,
                    name.as_str(),
                    scope_prefix,
...
fn extract_go_path_params(path: &str, params: &mut Vec<String>) {
...
fn controller_class_name_go(short: &str) -> String {
    let pascal = pascalize_word(short);
    format!("{pascal}Controller")
}
...
fn emit_go_routes(app: &App) -> EmittedFile {
    ...
    for route in &flat {
        let verb = match route.method.as_str() {
            "GET" => "Get",
            "POST" => "Post",
            "PUT" => "Put",
            "PATCH" => "Patch",
...
fn emit_go_render_call(arg: &Expr, ctx: &GoViewCtx) -> Vec<String> {
    match &*arg.node {
