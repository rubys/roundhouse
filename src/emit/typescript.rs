...
/// the schema SQL + seed registry, then calls `startServer` to
/// boot HTTP + Action Cable. Mirrors railcar's app.ts shape,
/// adapted to roundhouse's Router / schema / seeds surfaces.
fn emit_main_ts(app: &App) -> EmittedFile {
    ...
    writeln!(s).unwrap();
    // Seeds: if any fixture-like seed is emitted, import it here.
    // Today roundhouse doesn't emit a runnable seed loader — that's
    // a follow-up bite. Placeholder leaves room for it.
