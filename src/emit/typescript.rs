...
fn emit_model_file(model: &Model) -> EmittedFile {
    ...
    // `ApplicationRecord` is the abstract root. Import it from Juntos
    // directly; each concrete model extends it. Once Phase 3's
    // associations land, CollectionProxy / modelRegistry join this
    // import.
    writeln!(s, "import {{ ApplicationRecord }} from \"juntos\";").unwrap();
    writeln!(s).unwrap();
