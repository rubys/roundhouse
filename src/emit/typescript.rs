...
fn emit_tsconfig_json(app: &App) -> EmittedFile {
    ...
    if !app.controllers.is_empty() {
        includes.push_str(", \"app/controllers/**/*.ts\", \"src/http.ts\"");
    }
...
fn emit_controller_file(c: &Controller, known_models: &[Symbol]) -> EmittedFile {
    ...
    let mut s = String::new();
    writeln!(s, "// Generated by Roundhouse.").unwrap();
    // Imports only needed when at least one action references them,
    // but the controller is typically non-trivial; always emitting
    // is simpler and tsc accepts unused imports under non-strict.
    writeln!(s, "import * as Http from \"../../src/http.js\";").unwrap();
    ...
    let ctx = TsCtrlCtx { known_models };
    for action in &public_actions {
        emit_action(&mut s, action, &ctx, true);
    }
    for action in &private_actions {
        emit_action(&mut s, action, &ctx, false);
    }

    // Namespace object gathering all actions under the controller's
    // class name. Only the public actions go in — private helpers
    // stay file-scoped.
...
fn emit_ts_fixture(lowered: &crate::lower::LoweredFixture) -> EmittedFile {
    ...
    for record in &lowered.records {
        ...
        writeln!(
            s,
            "  const record = {class_name}.find<{class_name}>(id);"
        )
        .unwrap();
        writeln!(
            s,
