...
const RUNTIME_FILES: &[(&str, &str)] = &[
    ...
    // `view_helpers_generated.ts` (the transpile of
    // runtime/ruby/action_view/view_helpers.rb) is intentionally NOT
    // shipped: nothing in the emitted app imports it (every view
    // imports the hand-written `view_helpers.ts` instead), but tsc
    // still type-checks it under the include glob. The transpile is
    // currently incomplete (module-level state with `@slots`, Ruby
    // `string.gsub`, top-level constants like `HTML_ESCAPE_PATTERN`
    // not surfaced as exports) — shipping it just produced ~24 tsc
    // errors with no runtime payoff. The file remains generated by
    // `runtime_transpile_ts` so we can see emit progress on it; it
    // re-enters this list when the transpile pipeline catches up to
