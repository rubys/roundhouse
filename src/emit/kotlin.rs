...
pub fn emit(app: &App) -> Vec<EmittedFile> {
    ...
    if !app.test_modules.is_empty() {
        ...
        // `<Plural>Fixtures` objects — one per fixture YAML. Each exposes
        // per-label class methods (`ArticlesFixtures.one()` →
        // `Article.find(1)`) plus `_fixtures_load!` that RoundhouseTestCase
        // invokes after each schema reset. Registered so test bodies type
        // fixture reads (`@article = articles(:one)` infers Article).
        ...
        files.push(EmittedFile {
            path: std::path::PathBuf::from("src/test/kotlin/RhTestSupport.kt"),
...
/// `src/test/kotlin/TestSetup.kt` — the app-specific test wiring the
...
/// builders Main.kt uses). Empty-app entries no-op cleanly, so the
...
fn emit_test_setup(app: &App, fixture_lcs: &[crate::dialect::LibraryClass]) -> EmittedFile {
    ...
    let content = format!(
        "// Generated by Roundhouse (kotlin). App-specific test wiring —\n\
         // consumed by RoundhouseTestCase (RhTestSupport.kt).\n\
         \npackage roundhouse\n\
         \nobject RoundhouseTestSetup {{\n\
         \x20\x20\x20\x20const val schemaSql = \"{schema_lit}\"\n\
         \x20\x20\x20\x20val fixtureLoaders: List<() -> Unit> = listOf(\n{}\n    )\n\
    ...
    EmittedFile { path: std::path::PathBuf::from("src/test/kotlin/TestSetup.kt"), content }
}

/// The hand-written half of the test harness (the app-specific half is
/// the generated TestSetup.kt): RoundhouseTestCase — the JUnit-5 base
/// every emitted test class extends. Its `@BeforeEach` resets the
/// in-memory DB + reloads fixtures before every test (JUnit runs
/// superclass `@BeforeEach` methods before the subclass's emitted
/// `setup`); the controller-test surface (get/post/patch/delete +
/// assertResponse / assertRedirectedTo / assertSelect) dispatches
/// synchronously through the transpiled Router — the same path
/// Server.dispatch takes, minus HTTP. assertSelect is the substring
/// shim the crystal/ts/swift gates use (selector → opening-tag / id=
/// fragment). Failures report via JUnit `fail()`; the inlined minitest
/// assertions in test bodies throw RuntimeException, which JUnit
/// records as a failure per test (no trap problem — unlike Swift's
/// fatalError).
const RH_TEST_SUPPORT: &str = include_str!("../../runtime/kotlin/test_support.kt");
...
/// The routes-table + controller-factory-map Kotlin literal lines —
/// shared by Main.kt (production boot) and TestSetup.kt (controller-test
/// dispatch).
