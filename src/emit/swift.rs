...
pub fn emit(app: &App) -> Vec<EmittedFile> {
    ...
    if !app.test_modules.is_empty() {
        ...
        // The throwing-assertion support type the emitted tests use.
        files.push(EmittedFile {
            path: std::path::PathBuf::from("Tests/AppTests/RhTestSupport.swift"),
            content: "// Assertion-failure error for the transpiled minitest suites —\n\
                      // thrown by the inlined `raise \"…\" if cond` assertions; XCTest\n\
                      // reports the throwing test as failed with this description.\n\
                      struct RhTestFailure: Error, CustomStringConvertible {\n\
                      \x20\x20\x20\x20let message: String\n\
                      \x20\x20\x20\x20init(_ message: String) { self.message = message }\n\
                      \x20\x20\x20\x20var description: String { message }\n\
                      }\n"
                .to_string(),
        });
    }
...
fn emit_main(app: &App) -> EmittedFile {
    ...
    // Swift rejects an empty `[\n]` literal — a route-less app (the
    // framework-test fixtures) gets `[]` / `[:]`.
