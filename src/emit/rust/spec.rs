...
//! during Phase 7.2. The rust path filters `app.test_modules` to
//! `ActionDispatch::IntegrationTest`-parented modules (everything
//! else takes the lowerer-based model-test path in `rust.rs`),
...
/// file. The rust caller (`src/emit/rust.rs`) filters to
