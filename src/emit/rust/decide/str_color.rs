//! String-ownership coloring — rust's decide-pass module for
//! `STR_TO_OWNED` / `STR_BORROW` bits on `Expr.decisions`.
//!
//! Stamps each `Ty::Str` expression's `decisions` bits so the rust
...
//! borrowed vs owned strings. Relocated to `emit/rust/decide/`
...
/// Where a string lives at runtime once emitted by rust.
///
...
pub enum StrColor {
    ...
    /// returning `Ty::Str` (rust emits `-> String`).
    Owned,
}
...
/// Maps callable names to their rust string-position colors.
///
...
/// LibraryFunctions). Uses rust's emit conventions:
/// - `Ty::Str` parameter → `Borrowed` (rust emits `&str`)
/// - `Ty::Str` return → `Owned` (rust emits `String`)
///
...
pub fn build_registry(
    ...
    for class in classes {
        ...
        // `::`-segment). rust emit strips namespaces the same way
...
/// LibraryClass). rust's `emit_module` calls this when coloring
...
/// A `Ty::Str` parameter emits as `&str` in rust; everything else
...
/// A `Ty::Str` return emits as `String` in rust.
...
/// a `String`-emitted result in rust, regardless of receiver type.
...
/// The method name list is conservative: only methods whose rust
