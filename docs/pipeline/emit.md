...
the start. Python's entire app — models, views (jbuilder included),
controllers, dispatch, route table, fixtures, and the LC-emitted
unittest suite — rides its overlay (`src/emit/python/overlay.rs`,
emitted under `app/v2/` + `tests/`); the per-artifact emitters and
