...
// io << "..."` lowers to a StringBuilder; `#{...}` interpolations become
// Kotlin `${...}`; helper calls keep their lowered call shape
// (`ViewHelpers.*`, `RouteHelpers.*`, `Inflector.*`). The HTML string
// literals are reproduced verbatim — this is the byte-for-byte spec the
// emitter must reproduce.
object ArticlesView {
    fun index(articles: List<Article>, notice: String? = null, alert: String? = null): String {
        val io = StringBuilder()
        io.append("${ViewHelpers.turboStreamFrom("articles")}\n\n")
        ViewHelpers.contentForSet("title", "Articles")
        io.append("\n<div class=\"w-full\">\n")
        if (notice != null && notice.isNotEmpty()) {
            io.append("    <p class=\"py-2 px-3 bg-green-50 mb-5 text-green-500 font-medium rounded-md inline-block\" id=\"notice\">${ViewHelpers.htmlEscape(notice)}</p>\n")
        }
