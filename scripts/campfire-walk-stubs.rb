...
# into `show`, the second still runs from `process_action`, so it reads
# `@room` BEFORE anything sets it and dies on nil. Milestone walk finding #5,
# whose other half — an inlined filter losing its `return if performed?` —
# is a fail-open authorization bug on `destroy`.
WALK_LATE_PATCHES << lambda do
  require_relative "app/controllers/application_controller"

  class ApplicationController
    alias_method :walk_original_remember_last_room_visited, :remember_last_room_visited

    def remember_last_room_visited
      return if @room.nil? # ← would not be nil if the filters ran in order
      walk_original_remember_last_room_visited
    end
  end
end
